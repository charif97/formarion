...
// FIX: Import Express to resolve 'Cannot find namespace Express' error.
...
export interface NormalizedDocument {
    text: string;
    title: string;
    language: string; // Defaulting to 'en' for now
}
...
async function parseWebUrl(url: string): Promise<Partial<NormalizedDocument>> {
    ...
    // Basic extraction: get title and text from main content tags
    const title = $('title').first().text() || $('h1').first().text();
    $('script, style, nav, footer, header').remove(); // Remove non-content tags
...
async function parseYoutubeUrl(url: string): Promise<Partial<NormalizedDocument>> {
    ...
    // Fetch video title
    const videoInfoResponse = await axios.get(`https://noembed.com/embed?url=${encodeURIComponent(url)}`);
    const title = videoInfoResponse.data.title || "YouTube Transcript";
